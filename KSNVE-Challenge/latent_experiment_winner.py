...
def main(config, latent_size):
    ...
    encoder = latent.Encoder(in_channels=1)
    decoder = latent.Decoder(last_channel=1)
    model = latent.DeepSVDD_net(in_channels=1)
    pre_model = latent.AE1DCNN(encoder, decoder, config.sample_size, latent_size, 1).to(device)
    

    optimizer = Adam(model.parameters(), lr = config.learning_rate)
    loss = torch.nn.MSELoss()

    trainer = latent.TrainerDeepSVDD(pre_model, model, loss, optimizer, device, train_loader,
                                     eval_loader, is_pretrain=True)

    # trainer = funs.Trainer(model, loss, optimizer, device)
    trainer.pretrain(train_loader, config.epoch)
