...
def main(config, layer_size_list):
    ...
    model = pkgs.funs.AutoEncoder(pkgs.funs.LinearBlock, layer_size_list, in_channels = 2, input_size = config.sample_size).to(device)

    optimizer = Adam(model.parameters(), lr = config.learning_rate)
    loss = torch.nn.MSELoss()

    trainer = pkgs.funs.Trainer(model, loss, optimizer, device)
    ...
    # 2d tnse
    pkgs.latent.plot_tsne(config.tsne_root, latent_vectors, fault_labels, latent_size, 
